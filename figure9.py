...
def get_figure9_configs():
    ...
    for m in [
        ...
        for k, n in [
            (8192, 8192),(4096, 8192),(4096, 12288),
            (8192, 57344),
            (16384, 8192)
        ]:
            for (b_dtype, runners) in [
                ('float16', ['torch-f16']),
                # ('uint8', ['cutlass', 'triton', 'bitblas', 'mutis']),
                ('uint4b', ['marlin','tilelp','triton',  
                            'bitblas', 'mutis', 'cutlass']),
                ('uint2b', ['bitblas', 'mutis', 'tilelp']),
            ]:
...
def process(df: DataFrame, gpu: str, bs: int) -> DataFrame:
    ...
    runners = ['triton', 'quant-llm', 'bitblas', 'marlin', 'mutis', 'cuda', 'tilelp', 'cutlass']
...
def plot(df: DataFrame, out_fname):
    ...
    for ax_idx, (axes, bs) in enumerate(zip(axes_list, [1, 2])):
...
def analyze_speedup(df: DataFrame):
    ...
    competitors = ['triton', 'bitblas', 'quant-llm', 'marlin', 'tilelp']
...
def main():
    ...
    df_bs2 = process(df, bs=2, gpu=gpu)

    # Combine the processed dataframes for plotting
    combined_df = pd.concat([df_bs1, df_bs2])
