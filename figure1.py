...
def process(df: DataFrame, gpu: str, m_value: int) -> DataFrame:
    ...
    runners = [ 'torch-f16', 'triton', 'bitblas', 'marlin', 'mutis',   'gemlite']  # 移除 quant-llm
...
def plot_by_m(df: DataFrame, out_fname: str):
    ...
    fig.legend(handles, labels, bbox_to_anchor=(0.5, 0.98), loc='center',
               ncol=(len(executors) + 1)//2, fontsize=10)
    ax.axhline(y=4, color='red', linewidth=1.2, linestyle='-.',
                   label='Theoretical Speedup' if ax_idx == 0 else None)
...
def main():
    ...
    out_path = os.path.join(results_dir, f'gpu_{gpu}_figure1_by_m.pdf')
