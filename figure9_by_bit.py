    ...
    for m in [1, ]:
        for k, n in [
            ...
            for (b_dtype, runners) in [
                ('float16', ['torch-f16']),
                ('uint8', supported_runners_8bit),
                ('uint4b', supported_runners),
                ('uint2b', supported_runners_2bit),
...
def process(df: DataFrame, gpu: str, m_value: int) -> DataFrame:
    ...
    runners = [ 'torch-f16', 'triton', 'bitblas', 'marlin', 'mutis', 'tilelp', 'gemlite', 'quant-llm']  # 移除 quant-llm
...
def plot_by_m(df: DataFrame, out_fname: str):
    """Plot bars grouped by b_dtype on x-axis.
    Within each dtype group, only runners that have data are plotted,
    packed tightly with no gaps for missing runners.
    """
    ...
    for ax_idx, (ax, m_val) in enumerate(zip(axes, m_values)):
        ...
        bar_w = 0.12
        dtype_gap = 0.3   # extra gap between dtype groups
        added_labels = set()  # for legend de-duplication

        # Build x positions group by group
        group_centers = []   # center x for each dtype group (for xticks)
        current_x = 0.0

        for dt in dtypes:
            # Runners that actually have data for this dtype
            present = [e for e in all_executors
                       if len(df_m[(df_m['b_dtype'] == dt) & (df_m['runner'] == e)]) > 0]
            if not present:
                continue
            ...
            group_centers.append((group_start + current_x - bar_w) / 2)
            current_x += dtype_gap  # gap between groups
        ...
        ax.set_xticklabels(dtypes, ha='center')
        ax.set_ylabel('Speedup vs fp16')
        ax.grid(axis='y', alpha=0.3, linestyle='--', linewidth=0.5)
        ax.set_xlim(-bar_w, current_x)
