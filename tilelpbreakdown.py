...
@triton.jit
def gemm_splitK_INT_kernel_no_auto_tune(
    a_ptr, b_ptr, c_ptr,
    scales_ptr, 
    M, N, K, 
...
def gemm_splitK_forward_no_auto_tune(x: Tensor, W_q: Tensor, scales: Tensor, 
    ...
    kernel = gemm_splitK_INT_kernel_no_auto_tune[grid](
        x, W_q, output, 
        scales,  
        M, N, K, group_size,
        ###############################################
...
@triton.jit
def gemm_splitK_INT_kernel_no_auto_tune_no_evict(
    a_ptr, b_ptr, c_ptr,
    scales_ptr, 
    M, N, K, 
    group_size: tl.constexpr, 
    ...
    meta_evict_policy: tl.constexpr = '',
    atomic_mode: tl.constexpr = 'relaxed',
    a_evict: tl.constexpr = None,
    b_evict: tl.constexpr = None,
):
    """
    Based on https://github.com/foundation-model-stack/foundation-model-stack/blob/triton/triton/kernels/gptq/splitk_dequant_gemm.py
    GEMM for C = matmul(A, dequantize(B, scales, zeros))
    ...
    if(SPLIT_K > 1):
        ...
        tl.store(c_ptrs, acc, mask=mask)
def gemm_splitK_forward_no_auto_tune_no_evict(x: Tensor, W_q: Tensor, scales: Tensor, 
                         group_size: int,   elements_per_sample: int
                        ) -> Tensor: 
    ...
    kernel = gemm_splitK_INT_kernel_no_auto_tune_no_evict[grid](
        x, W_q, output, 
        scales,  
        M, N, K, group_size,
        ###############################################
