...
def dequanti_tensorRT_llm(b, scales):
    ...
    x1, x2, x3, x4, x5, x6, x7, x8 = tl.inline_asm_elementwise(
        asm="""
            ...
            // Split r9  -> h0 (lo), h1 (hi)
            mov.b32     {h0, h1}, r9;
            // Split r12 -> h2 (lo), h3 (hi)
            mov.b32     {h2, h3}, r12;
            // Split r16 -> h4 (lo), h5 (hi)
            mov.b32     {h4, h5}, r16;
            // Split r19 -> h6 (lo), h7 (hi)


            mov.b32     {h6, h7}, r19;
             // Load scale
            mov.b16 s, $9;
            // Multiply each half by scale
            mul.f16 h0, h0, s;
            mul.f16 h1, h1, s;
            mul.f16 h2, h2, s;
            mul.f16 h3, h3, s;
...
@triton.jit
def gemm_splitK_INT_kernel_small_batch(
    a_ptr, b_ptr, c_ptr,
    ...
    for k in range(num_pid_k):
        ...
        a_block = tl.reshape(a_block, (BLOCK_SIZE_M, BLOCK_SIZE_K,  2, 2, 2))
        a111, a211 = tl.split(a_block)
        a11, a21 = tl.split(a111)
        a31, a41 = tl.split(a211)
        a1, a2  = tl.split(a11)
        a3, a4 = tl.split(a21)
        a5, a6 = tl.split(a31)
        a7, a8 = tl.split(a41)        
        ...
        # Dot: accumulate 8 sub-block products
        acc = tl.dot(a1, b1, acc=acc, out_dtype=acc_dtype)
        acc = tl.dot(a2, b2, acc=acc, out_dtype=acc_dtype)
        acc = tl.dot(a3, b3, acc=acc, out_dtype=acc_dtype)
        acc = tl.dot(a4, b4, acc=acc, out_dtype=acc_dtype)
        acc = tl.dot(a5, b5, acc=acc, out_dtype=acc_dtype)
        acc = tl.dot(a6, b6, acc=acc, out_dtype=acc_dtype)
        acc = tl.dot(a7, b7, acc=acc, out_dtype=acc_dtype)
        acc = tl.dot(a8, b8, acc=acc, out_dtype=acc_dtype)
...
def gemm_splitK_INT_kernel(
    ...
    """
    Based on https://github.com/foundation-model-stack/foundation-model-stack/blob/
    triton/triton/kernels/gptq/splitk_dequant_gemm.py
    GEMM for C = matmul(A, dequantize(B, scales, zeros))
    A is of shape (M, K): float16 or bfloat16
    B is of shape (K//elements_per_sample, N): int32 as a packed matrix
    C is of shape (M, N): float16 or bfloat16 depending on the input A
    ...
    for k in range(num_pid_k):
        ...
        b = dequantize(b, scales, zeros, q_shift, meta_dtype, unpack_mask, 
                       elements_per_sample, W_group_mode, zero_is_scalar)
...
def gemm_splitK_forward(x: Tensor, W_q: Tensor, scales: Tensor, zeros: Tensor, scales_x: Tensor,
    ...
    if(is_mx_dtype(input_dtype)):
        ...
        if M == 64:
            gemm_splitK_kernel = gemm_splitK_INT_kernel_small_batch
        else:
